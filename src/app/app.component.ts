...
export class AppComponent {
  ...
  contactsForm = new FormGroup({
    ...
    salary: new FormControl('', [Validators.pattern(/^[0-9]+(\.[0-9]{1,2})?$/)]), // For numeric salary
  ...
  onFormSubmit() {
    ...
    if (this.contactToEdit) {
      const updateContactRequest = { id: this.contactToEdit.id, ...contactData };
      this.http.put(`${this.apiUrl}/${this.contactToEdit.id}`, updateContactRequest)
        .subscribe(() => this.refreshContacts());
    } else {
      this.http.post(this.apiUrl, contactData)
        .subscribe(() => this.refreshContacts());
    }
  ...
  onDelete(id: string) {
    ...
    if (confirmed) {
      this.http.delete(`${this.apiUrl}/${id}`).subscribe(() => this.refreshContacts());
    }
  }
  ...
  sortContacts(field: keyof Pick<Contact, 'name' | 'email'>) {
    ...
    this.filteredContacts$.next(this.sortArray([...this.filteredContacts$.getValue()]));
    this.updatePagination();
  }
